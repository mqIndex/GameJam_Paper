...
<<<<<<< Updated upstream
	...
	if ratio >= 0.75:
=======
	lbl_action_points.text = "行动力 %d / %d   %s" % [Game.action_points, Game.ACTION_POINTS_MAX, UF.ap_dots(Game.action_points, Game.ACTION_POINTS_MAX)]
	if Game.action_points == Game.ACTION_POINTS_MAX:
>>>>>>> Stashed changes
