...
# 跨关存档恢复缓冲: Saves.apply_to_game 写入, new_level 末尾消费一次
var pending_restore_deck_effect_ids: Array = []
var pending_restore_talent_ids: Array = []
var pending_restore_day_target: int = 0         # >0 = 恢复到第 N 天起点 (按 phase 选 PLAY/SHOP)
var pending_restore_phase_target: String = ""   # "play" | "shop" | ""(无快照)
var pending_restore_cash: float = 0.0
var pending_restore_shares: int = 0
var pending_restore_avg_cost: float = 0.0
var pending_restore_price: float = 0.0
var pending_restore_bull: int = 0
var pending_restore_turn_global: int = 0
# 上一次稳定恢复点快照 (仅在 _start_day / _enter_shop 末尾拍一次, 商店内购买不更新)
var snapshot_day: int = 0
var snapshot_phase: String = ""
var snapshot_cash: float = 0.0
var snapshot_shares: int = 0
var snapshot_avg_cost: float = 0.0
var snapshot_price: float = 0.0
var snapshot_bull: int = 0
var snapshot_turn_global: int = 0
var snapshot_deck_effect_ids: Array = []
var snapshot_talent_ids: Array = []
# 教学跳过门槛: tutorial_day <= 此值的所有 step 都会被 _build_steps 过滤掉
# 0 = 不跳过 (默认正常播); _apply_pending_restore_point 恢复到第 N 天商店时, 设为 N
var tutorial_skip_until_day: int = 0
# 连携效应运行时状态机 (per_turn 作用域, _start_turn 重置)
var combo_buy_count: int = 0
var combo_sell_count: int = 0
# 影响力运行时计数 (per_turn 作用域): 升降合并, 每 2 张让股价变动的牌触发一次
var combo_price_move_count: int = 0
var _in_cascade: bool = false                   # 重入保护: 连携 / 影响力 等额外触发期间, 不再二次计入额外触发
...
func new_level(carried_effect_ids: Array = []) -> void:
	...
	discard_pile.clear()
	disabled_pile.clear()
	if pending_restore_deck_effect_ids.is_empty():
		...
		# 从存档恢复: 整副牌完整重建 (忽略 carried, 因为档里已经包含全部)
		draw_pile = []
		for i in range(pending_restore_deck_effect_ids.size()):
			var eid: String = String(pending_restore_deck_effect_ids[i])
			var c: Card = CardDatabase.make_by_effect(eid, "restored_%s_%d" % [eid, i])
			if c != null:
				draw_pile.append(c)
		pending_restore_deck_effect_ids = []
	draw_pile.shuffle()
	# 天赋: 存档恢复优先, 没档则保留当前 owned_talents (跨关本身就保留)
	if not pending_restore_talent_ids.is_empty():
		owned_talents = []
		for tid in pending_restore_talent_ids:
			var t = TalentDatabase.make_by_id(String(tid))
			if t != null:
				owned_talents.append(t)
		pending_restore_talent_ids = []
	...
	emit_signal("state_changed")
	if pending_restore_phase_target != "":
		_apply_pending_restore_point()
	else:
		_start_day()
...
func play_card(index: int) -> bool:
	...
	# 天赋: 影响力 (influence) — 每 2 张让股价变动 (上升或下降, 不分方向) 的牌, 第 2 张额外触发一次
	# 资源不足时也照常额外触发, 始终不消耗行动力; 计数作用域 = 本回合
	if has_talent("influence"):
		if price != price_before:
			combo_price_move_count += 1
			if combo_price_move_count >= 2 and not _in_cascade:
				_try_influence_dispatch(c)
				combo_price_move_count = 0
...
# ---- 存档恢复点: _start_day / _enter_shop 末尾拍一次 ----
# 商店内买卡/买天赋/删卡都不更新, 防止"恶意退出回滚商店"
func _take_save_snapshot(phase_tag: String) -> void:
	snapshot_phase = phase_tag
	snapshot_day = day
	snapshot_cash = cash
	snapshot_shares = shares
	snapshot_avg_cost = avg_cost_price
	snapshot_price = price
	snapshot_bull = bull
	snapshot_turn_global = turn_global
	snapshot_deck_effect_ids = _collect_full_deck_effect_ids()
	snapshot_talent_ids = _owned_talent_ids()


func _collect_full_deck_effect_ids() -> Array:
	var ids: Array = []
	for c in get_full_deck():
		if c == null or c.transient:
			continue
		ids.append(String(c.effect_id))
	return ids


# new_level 之后, 若有 pending 收盘存档点 (phase == "shop"), 跳过默认 _start_day,
# 把 day/cash/价格 等顶回快照点, 然后直接进入该天的 _enter_shop (相当于第 N 天收盘后)
func _apply_pending_restore_point() -> void:
	cash = pending_restore_cash
	shares = pending_restore_shares
	avg_cost_price = pending_restore_avg_cost
	price = pending_restore_price
	bull = clamp(pending_restore_bull, 0, EMOTION_TOTAL)
	bear = EMOTION_TOTAL - bull
	turn_global = pending_restore_turn_global
	day = pending_restore_day_target
	turn_in_day = TURNS_PER_DAY
	cur_open = price
	cur_high = price
	cur_low = price
	day_open_price = price
	day_open_assets = get_total_assets()
	tutorial_skip_until_day = day
	_enter_shop()
	pending_restore_phase_target = ""
	pending_restore_day_target = 0
...
# 影响力: 与连携效应同机制 (额外触发一次, 不消耗行动力, 写独立 K)
# 资源不足时也照常触发: 调用 _dispatch_effect 不做前置门槛 (与 cascade 不同)
func _try_influence_dispatch(c: Card) -> void:
	var pb: float = price
	var hib: float = cur_high
	var lob: float = cur_low
	var bb: int = bull
	_in_cascade = true
	_log("  [影响力] 「%s」连续 2 张涨/跌, 额外触发一次" % c.name)
	_dispatch_effect(c.effect_id)
	_in_cascade = false
	_record_play_candle(pb, hib, lob, bb, "%s (影响力)" % c.name)
...
func _capture_turn_undo_snapshot() -> void:
	_turn_undo_snapshot = {
		...
		"combo_price_move_count": combo_price_move_count,
...
func _restore_turn_undo_snapshot() -> void:
	...
	combo_price_move_count = int(s.get("combo_price_move_count", 0))
